...
export default function ScenarioProcessLine({
  ...
  const { closeAllContainerConnection } = useContainerLogs()
  useEffect(() => {
    if (
  ...
  const items = [
    ...
    },
    {
      ...
      purpose: '指挥官下达攻击指令',
      description:
        '指挥官指示Agent进行黑盒/白盒攻击,攻击一段时间后,Agent会自行判断是否攻击完毕，指挥官也可以自行手动停止攻击->',
      action: {
        label: '停止攻击',
        onClick: () => {
          stopAttackerLogs()
          stopDefenderLogs()
          closeAllContainerConnection(scenarioId)
    ...
    },
    {
      ...
      description:
        '攻击/防御agent结束，指挥官可以与攻击Agent进行对话，但不会对靶机进行攻击,或者可以直接开始生成演练报告->',
      action: {
        label: '生成演练报告',
        onClick: () => {
          analyzeAsync()
    ...
    },
    {
      ...
      action: {
        ...
        onClick: () => {
          navigate({
            to: '/scenarios/$scenarioId/report',
            params: { scenarioId },
          })
        },
      },
  ...
  return (
    <ProcessLine
      lineItems={items}
      currentStep={step}
      className={cn(className, 'no-scrollbar overflow-y-auto')}
    />
  )
}
